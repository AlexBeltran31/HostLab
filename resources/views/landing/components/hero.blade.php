<section id="hero" class="h-screen flex items-center justify-center bg-gray-950 text-white px-6">
    <div class="max-w-4xl mx-auto text-center">

        <span class="text-sm font-medium tracking-widest text-amber-400 uppercase">
            {{ __('landing.hero_badge') }}
        </span>

        <h1 class="mt-2 text-3xl md:text-5xl font-bold leading-tight">
            {{ __('landing.hero_title') }}
        </h1>

        <p class="mt-4 text-base md:text-xl text-gray-300 max-w-2xl mx-auto">
            {{ __('landing.hero_subtitle') }}
        </p>

        <a href="#evaluacion"
           class="mt-6 inline-block bg-amber-400 text-gray-950 font-semibold px-6 py-2 rounded-full hover:bg-amber-300 transition">
            {{ __('landing.hero_cta') }}
        </a>

    </div>
</section>